...
public class TPCW_Database {
	...
	try {
	    ...
	    Connection con = getConnection();
	    PreparedStatement statement = con.prepareStatement
		("SELECT * FROM items WHERE items.i_subject = ? ORDER BY items.i_name limit 50");
	...
	try {
	    ...
	    Connection con = getConnection();
	    PreparedStatement statement = con.prepareStatement
		("UPDATE item SET i_cp = ?, i_image = ?, i_star = ?, i_prime = ? i_id = ?");
	...
	try {
		...
		PreparedStatement get_order_lines = con.prepareStatement
		    ("SELECT * " + "FROM order_line, items " + "WHERE ol_o_id = ? " +"AND ol_i_id = i_id");
	...
	try {
		...
		("SELECT i_stock FROM items WHERE i_id = ?");
	...
	try {
		...
		("UPDATE items SET i_stock = ? WHERE i_id = ?");
